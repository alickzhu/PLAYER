...
from Game_Framework import Game, Agent, ChatItem, EvaluateItem, ScriptManager

class Game_Werewolf(Game):
        ...
        for i in tqdm(range(len(self.agents))):
            for x,victim in enumerate(self.agents[i].victims):
                ...
                for j in tqdm(range(len(self.agents)), desc='Character {0} investigate {1}'.format(self.agents[i].name, victim)):
                    ...
                    if args.human and self.agents[i].kill_by_me[x] == 1:
                        question = input("请[{0}](你)提问[{1}]关于受害者[{2}]的问题，回车继续:\n".format(self.agents[i].name, self.agents[j].name, victim))
                        prompt2 = ''
                    else:
                    ...
                    if relay_agent.kill_by_me[x] == 1:
                        if args.human:
                            reply = input("请[{0}](你)回答[{1}]关于受害者[{2}]的问题，回车继续:\n".format(self.agents[j].name, self.agents[i].name, victim))
                            reply_prompt =""
                        else:
...
def main():
    ...
    game = Game_Werewolf(scriptmanager, args)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_history', default="/scratch/", type=str,required=False)
    ...
    parser.add_argument('--human', default=0, type=int, help='the max length for each agent\'s relative script length (retrival from current script by RAG')
    ...
    # for log
    parser.add_argument('--output_root_path', default=r'./log_human/Werewolf', type=str, required=False, help='max_tree_depth')
