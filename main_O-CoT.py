...
class Game:
    ...
    def update_script(self, n):
        ...
        # self.GM_agent.script = all_scrpt
        # file_name_faiss = os.path.join(args.database_root, args.script_name.split("-")[0],  ''.join(['all','_act_',str(n),'.faiss']))
        # file_name_doc = os.path.join(args.database_root, args.script_name.split("-")[0],  ''.join(['all','_act_',str(n),'.pickle']))
        # if os.path.exists(file_name_faiss):
        #     self.GM_agent.database.load_faiss(file_name_faiss)
        #     self.GM_agent.database.load_texts(file_name_doc)
        #
        # else:
        #     self.GM_agent.database.reset()
        #     self.GM_agent.database.add_text(self.GM_agent.script)
        #     if not os.path.exists(file_dir):
        #         os.makedirs(file_dir)
        #     self.GM_agent.database.save_faiss(file_name_faiss)
        #     self.GM_agent.database.save_texts(file_name_doc)
        #
...
# TODA LLAMA
    ...
    # for log
    parser.add_argument('--output_root_path', default=r'./log_O-CoT', type=str, required=False, help='max_tree_depth')
