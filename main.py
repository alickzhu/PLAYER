    ...
    def find_most_suspicion(self, i, x, victim, murderer = False, multi = True):
        ...
        for j in range(len(self.agents)):
            ...
            for i in range(len(self.sensors)):
                sensor_single = self.sensors[str(i)]
                ...
                summary_single.append(sensor_summary_single)
            summary_str = "About {character}:\n {summary}".format(character = self.agents[j].name, summary = '\n'.join(summary_single))
            summary_list.append(summary_str)

        summary = '\n'.join(summary_list)
        character_suspect = str(self.agents[i].character_suspect[x])
        number = math.ceil(len(self.agents[i].character_suspect[x]) / 2)

        # if self.args.mode:
        ask_question_prompt = self.prompts['s_constraint_multi'+postfix].replace('{character}',self.agents[j].name).replace('{summary}',summary).replace('{character_suspect}',character_suspect).replace('{victim}',victim)
        # else:
        #     ask_question_prompt = self.prompts['s_constraint'+postfix].replace('{character}',self.agents[j].name).replace('{summary}',summary).replace('{number}',str(number)).replace('{character_suspect}',character_suspect).replace('{victim}',victim)
        update_suspect = self.agents[i].speak(ask_question_prompt)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--script_name', default="131-Sin (4 people closed)", type=str, required=False)
